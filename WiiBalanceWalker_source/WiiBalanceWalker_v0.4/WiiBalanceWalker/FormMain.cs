...
namespace WiiBalanceWalker
{
    public partial class FormMain : Form
    {
        ...
        private void InfoUpdate()
        {
            ...
            {
                // Takes values from 0 to short.MaxValue (32767), where 1/2 * 32767 is the neutral position
                // Br typically moves between 30%-70%, so we take it squared to more appropriately fill the 0-100% interval.
                var joyX = 0.01 * brX * brX * 655.34;
                var joyY = 0.01 * brY * brY * 655.34;
                ...
                // Set new values.
                System.Diagnostics.Debug.WriteLine("joyX=" + joyX + ", joyY=" + joyY + ", brX=" + brX + ", brY=" + brY);
                joyDevice.SetAxis((short)joyX, 1, HID_USAGES.HID_USAGE_X);
                joyDevice.SetAxis((short)joyY, 1, HID_USAGES.HID_USAGE_Y);
            }
        }
        ...
        private void checkBox_EnableJoystick_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                joyDevice = new vJoy();
                joyDevice.AcquireVJD(1);
                joyDevice.ResetVJD(1);
            }
            catch (Exception ex)
            {
